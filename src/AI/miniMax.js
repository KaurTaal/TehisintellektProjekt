...
const miniMaxMove = (mängija, mängulaud, sügavus, teekond, esialgneMängija) => {
  ...
  for (let i = 0; i < käigud.length; i++) {
    ...
    tee.push(käik)
    harud.push(miniMaxMove(mängija === "valge" ? "must" : "valge", koopia, sügavus-1, tee, esialgneMängija));
  }

  let maxHaru = {hinne : losescore, tee : []};
  let minHaru = {hinne : winscore, tee : []};

  for (let i = 0; i < harud.length; i++) {
    if (harud[i].hinne >= maxHaru.hinne) {
      maxHaru = harud[i];
    }
    if (harud[i].hinne <= minHaru.hinne){
      minHaru = harud[i];
    }
  }

  if (esialgneMängija === mängija) {
    return maxHaru;
  }
  else {
    return minHaru;
